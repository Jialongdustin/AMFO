...
class nsynth_val():
    ...
    def Random_select_sample(self, args):
        ...
        sample_index_base = []
        final_index = []
        test_file = os.path.join(self.root, 'test.h5')
        for sess in range(self.sess + 1):
            index_inc = []
            sample_index_inc = []
            class_index_inc = []
            ...
            if sess == 0:
                ...
                for i in true_index_base:
                    sample_index_base = random.sample(index_base[i*100:(i+1)*100], 30)
                ...
                for i in class_index_inc:
                    sample_index_inc = random.sample(index_inc[i*100:(i+1)*100], 30)
    ...
    def Read_Index_Sess(self, sess):   
        voice = []
        label = []
        test_file = os.path.join(self.root, 'test128.h5')
        hf = h5py.File(test_file, 'r')
        f = open(self.IndexDir + '/test_' + str(sess + 1) + '.csv', 'r')
        reader = csv.reader(f)
        for i in reader:
